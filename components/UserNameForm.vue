<template>
    <form
        ...
        <input v-model="usernameModel" type="text" class="overflow-hidden pl-2 py-2 lowercase"
            :placeholder="placeholder" autocapitalize="off">
        <button :disabled="!usernameModel?.trim()" type="submit"
            :class="density === 'compact' ? 'my-1 mr-1' : 'my-2 mr-2'" class="flex-shrink-0 rounded border-transparent border-2 bg-black text-white
          hover:bg-white hover:text-black hover:border-black shadow transition-colors p-1">
            ...
            <span class="sr-only">Reveal User</span>
        </button>
    </form>
</template>
